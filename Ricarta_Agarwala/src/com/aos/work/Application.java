...
import com.aos.service.RAMutex;

public class Application {

	private Configuration resource;
	private Logger logger;
	private RAMutex lock;
	...
	public Application(Configuration config, Logger logger) throws IOException {
		...
		this.logger = logger;
		this.lock = new RAMutex(this.resource, this.logger);
	}
	...
	public synchronized void start() {
		...
		try {
			...
			for (int i = 0; i < numReq; i++) {
				...
				this.logger.writeClock(Arrays.toString(this.resource.getVectorClock().getVectorClock()));
				for (int j = 0; j < 2; j++)
