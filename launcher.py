...
class Launcher(Parser):
    ...
    def run_command(self):
        ...
        filePath = "" # Insert path where jar file is
        if self.nodes is None:
            logging.debug("node list is None")
            print "Parsing <id><host><port> failed"
            sys.exit(-1)

        out_name = self.CONFIG_FILE[:self.CONFIG_FILE.find('.txt')]
        # Should launch shell commands
        for i in range(0, len(self.nodes)):
            node = self.nodes[i]

            command = "ssh {0}@{1} \"cd {2};java -jar {3} {4} {5} >> " \
                      "SharedResource.txt\" &".format(
                self.net_id, node.get_host_name(), filePath, self.PROJ_NAME,
                self.CONFIG_FILE, node.get_id()
            )
...
if __name__ == "__main__":
    ...
    if os.path.isfile(os.path.join(os.getcwd(), launch.CONFIG_FILE)):
        logging.debug("Opened \'{0}\'".format(launch.CONFIG_FILE))
